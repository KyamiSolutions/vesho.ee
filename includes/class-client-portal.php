    ...
    <div class="vcl-card">
      ...
      <div class="vcl-card-body">
        ...
        <div class="vcl-panel active" id="vcl-panel-login">
          ...
          <?php
          $g_btn = '';
          $_gid  = get_option('vesho_google_client_id','');
          $_gsec = get_option('vesho_google_client_secret','');
          if ( $_gid && $_gsec ) {
              if ( class_exists('Vesho_Google_Auth') ) {
                  $g_btn = Vesho_Google_Auth::login_button('client');
              } else {
                  // Fallback: ehita URL otse
                  $_state = wp_create_nonce('vesho_google_client');
                  set_transient('vesho_google_state_'.$_state,'client',600);
                  $_redir = add_query_arg(['vesho_google_cb'=>'client'],home_url('/'));
                  $_aurl  = 'https://accounts.google.com/o/oauth2/v2/auth?'.http_build_query(['client_id'=>$_gid,'redirect_uri'=>$_redir,'response_type'=>'code','scope'=>'openid email profile','state'=>$_state,'access_type'=>'online','prompt'=>'select_account']);
                  $g_btn  = '<a href="'.esc_url($_aurl).'" class="vesho-google-btn"><svg width="18" height="18" viewBox="0 0 48 48" style="flex-shrink:0"><path fill="#EA4335" d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z"/><path fill="#4285F4" d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.65z"/><path fill="#FBBC05" d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24c0 3.88.92 7.54 2.56 10.78l7.97-6.19z"/><path fill="#34A853" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.15 1.45-4.92 2.3-8.16 2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z"/></svg>Jätka Google\'iga</a>';
              }
          }
          if ( current_user_can('manage_options') && !$_gid ) {
              echo '<p style="font-size:11px;color:#94a3b8;text-align:center;margin-top:8px">⚠️ Google: Client ID puudub seadetes</p>';
          }
          ?>
          <?php if ($g_btn): ?>
